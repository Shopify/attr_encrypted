...
class AttrEncryptedTest < Minitest::Test
  ...
  def test_should_store_email_in_encrypted_attributes_custom
    assert User.encrypted_attributes_custom.include?(:email)
  end

  def test_should_not_store_salt_in_encrypted_attributes_custom
    refute User.encrypted_attributes_custom.include?(:salt)
  end
  ...
  def test_attr_encrypted_should_not_use_the_same_attribute_name_for_two_attributes_in_the_same_line
    refute_equal User.encrypted_attributes_custom[:email][:attribute], User.encrypted_attributes_custom[:without_encoding][:attribute]
  end
  ...
  def test_should_decrypt_email_when_reading
    ...
    options = @user.encrypted_attributes_custom[:email]
  ...
  def test_should_inherit_encrypted_attributes_custom
    assert_equal [User.encrypted_attributes_custom.keys, :testing].flatten.collect { |key| key.to_s }.sort, Admin.encrypted_attributes_custom.keys.collect { |key| key.to_s }.sort
  end
  ...
  def test_should_not_inherit_unrelated_attributes
    ...
    assert SomeOtherClass.encrypted_attributes_custom.empty?
  end
  ...
  def test_should_work_with_aliased_attr_encryptor
    assert User.encrypted_attributes_custom.include?(:aliased)
  end
  ...
  def test_should_specify_the_default_algorithm
    assert YetAnotherClass.encrypted_attributes_custom[:email][:algorithm]
    assert_equal YetAnotherClass.encrypted_attributes_custom[:email][:algorithm], 'aes-256-gcm'
  end
  ...
  def test_encrypted_attributes_custom_state_is_not_shared
    user = User.new
    user.ssn = '123456789'

    another_user = User.new

    assert_equal :encrypting, user.encrypted_attributes_custom[:ssn][:operation]
    assert_nil another_user.encrypted_attributes_custom[:ssn][:operation]
  end
