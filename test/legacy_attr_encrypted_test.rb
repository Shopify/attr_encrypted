...
class LegacyAttrEncryptedTest < Minitest::Test
  ...
  def test_should_store_email_in_encrypted_attributes_custom
    assert LegacyUser.encrypted_attributes_custom.include?(:email)
  end

  def test_should_not_store_salt_in_encrypted_attributes_custom
    assert !LegacyUser.encrypted_attributes_custom.include?(:salt)
  end
  ...
  def test_attr_encrypted_should_not_use_the_same_attribute_name_for_two_attributes_in_the_same_line
    refute_equal LegacyUser.encrypted_attributes_custom[:email][:attribute], LegacyUser.encrypted_attributes_custom[:without_encoding][:attribute]
  end
  ...
  def test_should_inherit_encrypted_attributes_custom
    assert_equal [LegacyUser.encrypted_attributes_custom.keys, :testing].flatten.collect { |key| key.to_s }.sort, LegacyAdmin.encrypted_attributes_custom.keys.collect { |key| key.to_s }.sort
  end
  ...
  def test_should_not_inherit_unrelated_attributes
    ...
    assert LegacySomeOtherClass.encrypted_attributes_custom.empty?
  end
  ...
  def test_should_work_with_aliased_attr_encryptor
    assert LegacyUser.encrypted_attributes_custom.include?(:aliased)
  end
