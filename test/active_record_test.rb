...
class Account < ActiveRecord::Base
  ...
  def encrypting?(attr)
    encrypted_attributes_custom[attr][:operation] == :encrypting
  end
...
class ActiveRecordTest < Minitest::Test
  ...
  def test_should_allow_proc_based_mode
    ...
    assert_equal @person.class.encrypted_attributes_custom[:email][:mode].class, Proc
    assert_equal @person.class.encrypted_attributes_custom[:email][:mode].call, :per_attribute_iv_and_salt
    refute_nil @person.encrypted_email_salt
    refute_nil @person.encrypted_email_iv

    # Credentials is :single_iv_and_salt
    assert_equal @person.class.encrypted_attributes_custom[:credentials][:mode].class, Proc
    assert_equal @person.class.encrypted_attributes_custom[:credentials][:mode].call, :single_iv_and_salt
